            ...
            <tr>
                ...
                if(isset($collectorsresult)) {
                    foreach ($collectorsresult as $item ) {
                        ...
                        echo '<td>' . '<img style="height:150px;width:150px;" src="https://localhost/SEM2/LR1/inc/images/' . htmlspecialchars($item['img_path']) . '"/>' . '</td>';
                        echo '<td>' . htmlspecialchars($item['name']) . '</td>';
                        echo '<td>' . htmlspecialchars($item['birth_date']) . '</td>';
                        echo '<td>' . htmlspecialchars($item['characteristic']) . '</td>';
                        echo '<td>' . htmlspecialchars($crewsresult[$item['id_crew']]['crew']) . '</td>';
                        echo '<td>' . '<a class="btn btn-primary" type="button" id="edit" href="?action=edit&editid=' . htmlspecialchars($item['id']) . '">Редактировать</a>' . '</td>';
                        echo '<td>' . '<a type="button" onclick="return deletionCheck()" class="btn btn-danger delete" href ="?collectorsdeleteid=' . htmlspecialchars($item['id']) . '">Удалить</a>' . '</td>';
                        echo '</tr>' . " ";
                    }
                }
                ?>
