            ...
            <tr>
                ...
                if(isset($crewsresult))
                    foreach ($crewsresult as $item) {
                        ...
                        echo '<td>'.'<a  type="submit"  href="/SEM2/LR1/collectors.php?crewfilter='.htmlspecialchars($item['id']).'">'.htmlspecialchars($item['crew']).'</a>' .'</td>';
                        echo '<td>'.'<a class="btn btn-primary" type="submit"  href="?action=edit&editid='.htmlspecialchars($item['id']).'">Редактировать</a>' .'</td>';
                        echo '<td>'.'<a type="submit" onclick="deletionCheck();"  class="btn btn-danger delete" href ="?crewsdeleteid='.htmlspecialchars($item['id']).'&img='.htmlspecialchars($item['img_path']).'">Удалить</a>'.'</td>';
                        echo '</tr>' . " ";
                    }
                ?>
            </tr>
